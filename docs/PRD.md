...
| V1.2 | 2026-07-12 | AI产品经理 | 测评体系重构：弃用MBTI模型，改用IPIP大五人格+霍兰德RIASEC组合方案，规避商标侵权风险；新增三层人格标签体系与243种原型配置 |
...
**免费层：**
- 完成测评后立即获得三层人格标签：人格画像名（如"VSN-FHR"）、RIASEC职业兴趣码（如"IAS"）、五维色彩光谱码、基础雷达图及简要生活建议
- 支持一键生成高清认证卡并分享至社交媒体
...
| **用户体验** | 16Personalities以角色化命名和插画体系取胜 | 设计243种英文标签人格原型+色彩光谱码，更具本土化传播力 | [五、发展趋势与产品设计启示] |
...
**功能描述：**  
- 输入：用户点击"开始测试"按钮  
- 处理：展示引导弹窗，包含测评时长（约10分钟）、题目数量（80题）、科学背书说明（"基于IPIP大五人格量表与霍兰德RIASEC职业兴趣理论"）、七点李克特量表说明  
...
**功能描述：**  
- 输入：用户通过7点量表选择符合程度（1=完全不符合 ~ 7=非常符合）  
- 处理：单题全屏显示，每道题为陈述句+七点李克特量表选择；禁用浏览器返回键防止误触；记录作答时间戳  
- 输出：选择后自动切换至下一题，更新本地答题记录  
- 字段定义：question_id, statement_text, scale_value(1-7), response_timestamp  

**验收标准：**  
- 支持7点量表按钮选择，选中后350ms自动跳转下一题  
- 支持键盘1-7快捷键映射  
- 操作响应延迟<0.3秒  
- 屏幕亮度自动锁定，防止自动熄屏中断作答  
...
**功能描述：**  
- 输入：用户提交全部答案  
- 处理：调用评分引擎计算大五人格五维度得分（O/C/E/A/N百分位），映射为243种人格原型之一，同时计算RIASEC六维度得分取前三生成职业兴趣码，生成五维雷达图  
...
#### 功能名称：夜览模式  
**优先级：P1**  
**用户故事：** 作为一个长时间使用应用的用户，我希望在夜间使用时能切换到暗色主题，减少屏幕亮度对眼睛的刺激  
**功能描述：**  
- **夜览模式**：全站支持暗色主题切换，首次访问跟随系统偏好，用户手动切换后持久化存储到 localStorage。导航栏提供月亮/太阳图标一键切换。  
...
|------|--------|----------|-----------|---------|
| 大五人格（IPIP-50） | 50题 | O/C/E/A/N | 各10题 | 7点李克特量表，部分反向计分 |
| 职业兴趣（RIASEC-30） | 30题 | R/I/A/S/E/C | 各5题 | 7点李克特量表，正向计分 |
| **合计** | **80题** | **11个维度/类型** | - | - |

#### 7.2.2 量表说明
- **评分方式：** 7点李克特量表（1=完全不符合，2=比较不符合，3=有些不符合，4=不确定，5=有些符合，6=比较符合，7=非常符合）
...
| 模块 | 维度 | 题项示例 | 计分方向 |
|------|------|---------|---------|
| 大五人格 | 开放性(O) | "你喜欢思考新的想法和可能性" | 正向 |
| 大五人格 | 开放性(O) | "你更倾向于按部就班而非尝试新方法" | 反向 |
| 大五人格 | 尽责性(C) | "你会提前制定计划并按计划执行" | 正向 |
| 大五人格 | 外向性(E) | "在社交场合中你通常是主动交谈的人" | 正向 |
| 大五人格 | 宜人性(A) | "即使意见不同，你也能理解对方的立场" | 正向 |
| 大五人格 | 神经质(N) | "你经常感到焦虑或不安" | 正向 |
| RIASEC | 现实型(R) | "你喜欢动手修理或组装物品" | 正向 |
| RIASEC | 研究型(I) | "你喜欢分析复杂数据寻找规律" | 正向 |
| RIASEC | 艺术型(A) | "你喜欢通过创意作品表达自我" | 正向 |
| RIASEC | 社会型(S) | "帮助他人成长让你感到充实" | 正向 |
| RIASEC | 企业型(E) | "你喜欢带领团队达成目标" | 正向 |
| RIASEC | 常规型(C) | "你喜欢整理信息让一切井井有条" | 正向 |
...
| 第二层 | 人格画像名 | 大五五维度高/中/低三分组合映射 | `VSN-FHR` | 243种原型 |
...
大五人格五维度各分为高(H)/中(M)/低(L)三档，组合产生 3^5 = 243 种原型。以下为族群概览配置表，完整 243 种原型见 `人格原型扩展方案-243型.md`：

#### 7.4.1 高开放性组（O↑，共16种）

| 编号 | 维度组合 | 画像名 | 一句话描述 | 稀有度 | 典型职业方向 |
|------|---------|--------|-----------|--------|-------------|
| 01 | O↑C↑E↑A↑N↓ | VSN-FHR | 充满灵感且善于落地的领导者 | 稀有 | 产品总监、创业CEO |
| 02 | O↑C↑E↑A↑N↑ | VSN-WSU | 敏锐感知他人需求的开拓者 | 极稀有 | 用户体验研究、心理咨询 |
| 03 | O↑C↑E↑A↓N↓ | 自信开拓者 | 目标明确的创新执行者 | 常见 | 市场总监、投资经理 |
| 04 | O↑C↑E↑A↓N↑ | 锐意进取者 | 追求卓越的竞争型创新者 | 常见 | 管理咨询、投行分析师 |
| 05 | O↑C↑E↓A↑N↓ | VSN-SRN | 深思熟虑的系统设计者 | 稀有 | 系统架构师、战略规划 |
| 06 | O↑C↑E↓A↑N↑ | 深思关怀者 | 细腻负责的知识工作者 | 常见 | 学术研究、医疗诊断 |
| 07 | O↑C↑E↓A↓N↓ | VSN-FRV | 专注高效的独立问题解决者 | 常见 | 软件工程师、数据科学家 |
| 08 | O↑C↑E↓A↓N↑ | 完美探索者 | 追求极致的深度思考者 | 稀有 | 科研学者、精算师 |
| 09 | O↑C↓E↑A↑N↓ | 灵感传播者 | 活力四射的创意传播人 | 常见 | 品牌策划、内容创作 |
| 10 | O↑C↓E↑A↑N↑ | INS-FHR | 感性丰富的社交达人 | 常见 | 公关、社群运营 |
...
| 15 | O↑C↓E↓A↓N↓ | AER-FRV | 冷静超然的自由思想者 | 常见 | 独立学者、自由职业 |
...
| 编号 | 维度组合 | 画像名 | 一句话描述 | 稀有度 | 典型职业方向 |
|------|---------|--------|-----------|--------|-------------|
| 17 | O↓C↑E↑A↑N↓ | STD-CSK | 务实可靠的团队凝聚者 | 常见 | 项目经理、HR总监 |
| 18 | O↓C↑E↑A↑N↑ | STD-WSU | 细心负责的关怀者 | 常见 | 护理管理、教育培训 |
| 19 | O↓C↑E↑A↓N↓ | 果断执行者 | 高效直接的行动派 | 常见 | 运营总监、销售管理 |
| 20 | O↓C↑E↑A↓N↑ | 务实竞争者 | 结果导向的务实竞争者 | 常见 | 房地产、快消品管理 |
| 21 | O↓C↑E↓A↑N↓ | STD-SRN | 踏实可靠的幕后支柱 | 常见 | 财务会计、行政管理 |
| 22 | O↓C↑E↓A↑N↑ | 忠诚守卫者 | 尽职尽责的守护者 | 常见 | 品质管理、安全审核 |
| 23 | O↓C↑E↓A↓N↓ | STD-FRV | 理性客观的问题解决者 | 常见 | 数据分析、工程管理 |
| 24 | O↓C↑E↓A↓N↑ | 严谨审查者 | 一丝不苟的质检专家 | 稀有 | 审计师、法务合规 |
| 25 | O↓C↓E↑A↑N↓ | 活跃助人者 | 热心肠的行动派 | 常见 | 社区服务、活动策划 |
| 26 | O↓C↓E↑A↑N↑ | GRD-FHR | 感性热心的社交爱好者 | 常见 | 客户服务、社团组织 |
...
| 31 | O↓C↓E↓A↓N↓ | CTN-GLC | 随遇而安的自由灵魂 | 常见 | 自由职业、gap year |
...
| 字段名 | 类型 | 说明 | 示例 |
|--------|------|------|------|
| archetype_id | int | 原型编号(1-243) | 1 |
| archetype_code | string | 维度组合码 | OHCHEHAEHLN |
| archetype_name | string | 英文标签画像名 | VSN-FHR |
...
#### 7.5.1 大五人格评分流程
```
1. 收集50道大五题项的原始作答（1-7分）
2. 对反向计分题进行翻转（1↔7, 2↔6, 3↔5, 4↔4）
3. 计算每个维度的总分（10题之和，范围10-70）
4. 转换为百分位数（基于常模数据）
5. 按三分位划分高(H)/中(M)/低(L)三档
6. 将五维度高中低组合映射到243种原型之一
```

#### 7.5.2 RIASEC评分流程
```
1. 收集30道RIASEC题项的原始作答（1-7分）
2. 计算每个类型的总分（5题之和，范围5-35）
3. 按总分降序排列六个类型
...
| 题项数量 | 60题（二分强制选择） | 80题（7点李克特量表） |
| 评分方式 | 二分法（E/I, S/N, T/F, J/P） | 连续分数（百分位） |
| 标签格式 | INTJ-A-C | VSN-FHR·IAS + 色彩光谱 |
| 类型数量 | 16种×2变体=32 | 243原型×120兴趣码=29160种组合 |
