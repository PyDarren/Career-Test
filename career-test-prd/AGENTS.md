...
职探是一款面向大学生和职场新人的轻量化 MBTI 职业性格测评网页。核心体验是"八分钟，看见你的职业性格"——用户无需注册，打开即测，3–5 分钟完成 48 道 6 点刻度迫选题，免费查看人格认证卡，付费 2.99 元解锁 12 章深度报告。
...
| 文档 | 路径 | 说明 |
|------|------|------|
| 产品需求文档 | `PRD.md` | 产品功能、商业模式、用户画像等 |
| 技术设计文档 | `TECH_DESIGN.md` | 架构、数据库、API、安全设计等 |
...
├── manage.py
├── apps/
│   ├── assessment/        # 测评模块（题目、评分引擎、结果）
│   ├── mbti_types/        # MBTI 类型配置（16 型、认知功能栈）
│   ├── careers/           # 职业数据库与匹配算法
│   ├── payment/           # 支付模块（微信/支付宝、订单）
│   ├── stats/             # 统计模块
│   └── common/            # 公共模块（指纹、限流、工具函数）
├── templates/
│   ├── base.html
│   ├── pages/             # 页面模板
│   └── partials/          # 可复用组件模板
