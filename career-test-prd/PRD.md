...
- [测评体系设计](#assessment-design)
- [量表题库与评分算法规格](#scoring-spec)
- [职业匹配数据库](#career-database)
- [功能需求](#features)
- [深度报告内容模板](#report-template)
- [非功能性需求](#non-functional-requirements)
- [商业模式](#business-model)
- [数据指标](#metrics)
- [风险分析与应对](#risk-analysis)
- [冷启动运营策略](#launch-strategy)
...
| 9 | 报告访问凭证 | 支付成功后，深度报告访问凭证与用户 UUID 绑定，有效期 90 天。用户在有效期内可通过同一设备/浏览器重新访问报告页。凭证过期后展示"报告已过期，需重新测评并购买"提示。用户清除浏览器数据后无法自动恢复，提供凭支付凭证联系客服人工恢复的入口。 |
| 10 | 用户反馈 | 基础结果页推荐职业旁设置"推荐不准"反馈按钮，用户点击后选择原因（"方向完全不对"/"部分匹配"/"其他"）。深度报告底部设置"报告有帮助吗？"评价（👍/👎）+ 可选文字反馈（限 200 字）。反馈数据用于职业数据库校准和报告内容优化。 |
| 11 | 帮助中心 | 独立页面（/help/），包含 6–8 条常见问题：测评原理是什么、结果准确吗、为什么推荐职业不匹配、深度报告包含什么、支付后看不到报告怎么办、如何重新查看已购报告、如何清除我的数据、联系方式。页面支持搜索和折叠展开。 |
| 12 | 数据清除 | 设置页（/settings/）提供"清除我的数据"功能。清除内容包括：浏览器 localStorage 中的答题进度、UUID、测评结果缓存。清除后弹窗确认"已清除所有本地数据"，用户重新访问将作为新用户处理。服务端订单数据依法保留，不在清除范围内。 |
| 13 | 客服联系 | 支付弹窗、支付失败弹窗、帮助中心、深度报告页底部均设置"联系客服"入口。点击后展示客服微信号（支持长按复制）和在线留言表单（微信号/问题描述，提交后存入数据库）。客服工作日 9:00–18:00 回复。 |
| 14 | 分享回流落地页 | 用户通过分享链接访问时，落地页顶部展示"你的朋友 XXX 测出了 ENFP，来测测你的吧"引导卡片，3 秒内展示"开始测评"按钮。若分享链接携带 MBTI 类型参数，落地页可展示该类型的简要介绍卡片吸引点击。 |
| 15 | SEO 优化 | 每个页面配置独立的 title/description/og 标签。首页 title"职探 — 8 分钟 MBTI 职业性格测评"，结果页 title"我是 ENFP 竞选者，来测测你的 MBTI 职业性格"。添加 JSON-LD 结构化数据标记测评工具类型。sitemap.xml 和 robots.txt 自动生成。 |
...
以下功能不直接属于核心测评流程，但对用户体验闭环、数据质量保障和合规要求至关重要。按业务阶段分类说明。

#### 报告访问凭证管理

| 环节 | 规则 |
|------|------|
| 凭证生成 | 支付回调成功后，将订单号与用户 UUID 绑定，写入 `order` 表 `paid_at` 字段。前端同时将 `order_no` 存入 localStorage `paid_reports` 列表 |
| 凭证校验 | 访问 `/report/<order_no>/` 时，后端校验订单状态为 `paid` 且 `paid_at` 距今 ≤ 90 天。通过则渲染报告，否则展示过期提示页 |
| 凭证恢复 | 用户清除浏览器数据后，UUID 丢失。提供"输入支付订单号找回报告"入口，后端通过订单号查到关联 UUID，重新生成 UUID 写入用户 localStorage |
| 过期处理 | 超过 90 天的订单标记为 `expired`，报告页展示"报告已过期，重新测评仅需 2.99 元"并提供测评入口 |
| 多设备限制 | 同一订单仅允许从单个设备访问。若检测到不同设备（不同浏览器指纹）访问同一订单，展示"该报告已在其他设备查看，如需迁移请联系客服" |
...
基础结果页
  └─ 推荐职业标签旁 [反馈] 按钮
       ├─ "方向完全不对" → 记录 career_id + mbti_type + feedback_type=mismatch
       ├─ "部分匹配"     → 记录 career_id + mbti_type + feedback_type=partial
       └─ "其他"         → 弹出文字输入框（限 100 字）→ 记录到 feedback 表

深度报告页底部
  ├─ "报告有帮助吗？" [👍] [👎]
  │    └─ 记录 order_no + mbti_type + rating=up/down
  └─ "说说你的感受"（可选，限 200 字）
       └─ 记录 order_no + mbti_type + content
```

- **数据用途**：推荐不准反馈累计超过某职业总展示量的 5% 时触发该职业数据审查；报告评价 👎 占比超过 20% 时触发报告内容审查
- **反馈频率**：同一用户同一结果页仅允许提交 1 次反馈，防止刷量

#### 帮助中心（/help/）

| 问题编号 | 问题 | 回答要点 |
|---------|------|---------|
| Q1 | 测评原理是什么？ | 介绍 MBTI 四维度模型、6 点刻度迫选法、维度面向理论、认知功能理论的简要说明 |
| Q2 | 测评结果准确吗？ | 说明重测信度 ≥ 0.85 的指标、48 题题量与标准版 93 题的对比、6 点刻度法的精度优势。附免责声明 |
| Q3 | 为什么推荐的职业不适合我？ | 说明职业匹配算法的原理（类型匹配 + 维度强度匹配），建议结合实际体验判断，可点击"推荐不准"反馈帮助优化 |
| Q4 | 深度报告包含什么内容？ | 列出 12 章内容概览，配预览截图 |
| Q5 | 支付后看不到报告怎么办？ | 引导检查支付状态（提供订单号查询入口），若支付成功仍无法查看则联系客服 |
| Q6 | 如何重新查看已购报告？ | 说明 90 天有效期内可通过同一浏览器访问，清除数据后可通过订单号找回 |
| Q7 | 如何清除我的数据？ | 引导至设置页 /settings/，说明清除范围（本地数据）和保留范围（支付记录） |
| Q8 | 联系方式 | 客服微信号 + 工作时间 + 在线留言入口 |

#### 数据清除（/settings/）

| 清除项 | 存储位置 | 清除后影响 |
|--------|---------|-----------|
| 答题进度 | localStorage `assessment_progress` | 无法恢复未完成测评 |
| 用户 UUID | localStorage `user_uuid` | 丢失测评历史关联，无法查看已购报告（需订单号找回） |
| 测评结果缓存 | localStorage `last_result` | 下次访问需重新提交评分 |
| 付费报告列表 | localStorage `paid_reports` | 无法从报告列表快速跳转（不影响服务端凭证） |
| 浏览器指纹 | Canvas Fingerprint 为实时生成，无法单独清除 | — |

> 清除操作需二次确认弹窗："确定清除所有本地数据？清除后将无法恢复测评进度和历史记录。"确认后逐项清除并展示完成状态。

#### 客服联系

| 入口位置 | 展示形式 | 响应方式 |
|---------|---------|---------|
| 支付弹窗底部 | "遇到问题？联系客服"文字链接 | 弹出客服微信 + 留言表单 |
| 支付失败弹窗 | "联系客服"按钮 | 同上 |
| 帮助中心 Q8 | 客服微信号 + 留言表单 | — |
| 深度报告页底部 | "报告有问题？反馈给我们" | 跳转反馈入口 |

> 在线留言表单字段：微信号或手机号（选填）、问题描述（必填，限 500 字）、相关订单号（选填）。提交后存入 `customer_service_message` 表，客服后台可见。

#### 分享回流落地页

| 参数 | 说明 | 示例 |
|------|------|------|
| `ref` | 分享者 UUID（用于统计回流来源） | `?ref=550e8400...` |
| `type` | 分享者 MBTI 类型（用于展示引导卡片） | `&type=ENFP` |
| `name` | 分享者昵称或类型名（可选，保护隐私） | `&name=竞选者` |

- 落地页顶部展示引导卡片："你的朋友测出了 ENFP 竞选者，8 分钟测出你的职业性格"
- 卡片下方 3 秒后展示"开始测评"按钮 + 倒计时动效
- 若 `type` 参数存在，卡片右侧展示该类型的 3D 人偶缩略图
- 分享回流用户的新增 UV 计入"分享回流率"指标

#### SEO 与页面 Meta 信息

| 页面 | title | description | og:image |
|------|-------|-------------|----------|
| 首页 | 职探 — 8 分钟 MBTI 职业性格测评 | 免注册、免费测、8 分钟出结果。48 题 6 点刻度 MBTI 测评，免费查看人格认证卡，解锁深度职业性格报告。 | 默认品牌图 |
| 答题页 | 开始测评 — 职探 MBTI 职业性格 | 48 道精心设计的 MBTI 题目，6 点刻度迫选法，6-8 分钟发现你的职业性格。 | — |
| 结果页 | 我是 ENFP 竞选者，来测测你的 MBTI 职业性格 | 我在职探测出了 ENFP 竞选者人格，稀有度 8.49%。8 分钟测出你的 MBTI 职业性格，免费查看人格认证卡。 | 认证卡分享图 |
| 报告页 | ENFP 深度职业性格报告 — 职探 | 12 章 MBTI 深度报告：人格特征、优势劣势、荣格八维、恋爱专题、职业推荐。 | — |
| 帮助页 | 帮助中心 — 职探 | 常见问题解答：测评原理、结果准确性、支付问题、数据隐私等。 | — |

> 首页添加 JSON-LD 结构化数据标记（`@type: WebApplication`，`applicationCategory: LifestyleApplication`），提升搜索引擎理解产品类型。`sitemap.xml` 包含首页、答题页、帮助页、设置页（结果页和报告页因动态生成不纳入 sitemap）。
...
> **定价逻辑：** 2.99 元的定价低于一瓶饮料的价格，极大降低学生群体的付费心理门槛。远低于北森等专业平台的 99–199 元和知己 MBTI 的 19–39 元，以"几乎不犹豫"的定价策略追求高转化率。单一产品降低用户选择成本——无需在多个档位之间纠结，看到基础结果后一步即可解锁全部内容。详细报告内容模板见[深度报告内容模板](#report-template)章节。
